...
const categories = ['All', 'Case Studies', 'Brand Videos', 'Social Ads'];

const PortfolioPage: React.FC = () => {
  const [activeCategory, setActiveCategory] = useState('All');

  const filteredItems: PortfolioItem[] = activeCategory === 'All'
    ? PORTFOLIO_ITEMS
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
      <div className="text-center">
        ...
        <p className="mt-4 max-w-3xl mx-auto text-lg sm:text-xl text-slate-600 dark:text-slate-400">
          Explore a collection of stunning videos created by our community and for our clients.
        </p>
      </div>
      ...
      <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredItems.map(item => (
          <div key={item.id} className="group cursor-pointer rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 bg-white dark:bg-slate-900">
            <div className="relative aspect-video">
              <img src={item.videoUrl} alt={item.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
